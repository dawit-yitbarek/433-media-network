...
const PostPage = () => {
    ...
    const { gradient, accent, title, icon } =
        categoryStyles[category] || categoryStyles["sport"];

    return (
        <>
            <SEOHead
                title={`${post?.title} - 4-3-3 Media Network`}
                description={
                    post?.content?.slice(0, 150) ||
                    "Read more from 4-3-3 Media Network."
                }
            ...
            <section
                className={`min-h-screen text-[#EAEAEA] bg-gradient-to-b ${gradient} relative overflow-hidden`}
            >
                {/* Hero Section */}
                <div className="max-w-6xl mx-auto mt-24 px-4 md:px-8 flex flex-col lg:flex-row gap-10 items-start">
                    ...
                    <div className="lg:w-1/2 w-full">
                        <span
                            className="flex gap-3 items-center text-sm font-semibold uppercase tracking-widest"
                            style={{ color: accent }}
                        >
                            {icon}
                            {title}
                ...
                <div className="max-w-4xl mx-auto px-4 md:px-8 pb-20">
                    ...
                    <article className="prose prose-invert prose-lg max-w-none leading-relaxed text-[#D0D4E0]">
                        {post?.content.split("\n").map((p, i) => (
                            <p
                                key={i}
                                className="mb-5 text-base md:text-lg font-light"
                            >
                                {p}
                            </p>
                        ))}
