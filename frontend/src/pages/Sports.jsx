      ...
      <section>
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-8">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              <RotateCcw size={20}
                className={`${fetchingMatch ? "animate-spin" : ""}`}
              />
            </button>
          </div>
        </div>
        ...
        {!fetchingMatch && fetchingMatchError && (
          <TableError message={fetchingMatchError} refresh={() => setRefresh((prev) => prev + 1)} />
        )}


        {/* 3️⃣ Empty State */}
        {!fetchingMatch && !fetchingMatchError && leagues.length === 0 && (
          <EmptyMatchTable selectedDate={selectedDate} matchDateDisplay={matchDateDisplay} />
        )}


        {/* 4️⃣ Leagues Display */}
        {!fetchingMatch && !fetchingMatchError && leagues.length > 0 && (
          <MatchTable leagues={leagues} />
        )}
      </section>
